...
```{r}
left_join(x=states, y=regions, by=join_by(state == state))
```

b.  Perform an inner join. How many rows remain, and why is it the same or different?

```{r}
inner <- inner_join(x=states, y=regions)
dim(inner)
```
...
```{r}
#d: left join. those do not have a matching are excluded so there'll be missing data if any typos occur when using inner_join. however, those rows/columns would be kept if using left_join.

#e
pivot_longer(states, c("Income", "Population", "Illiteracy"), names_to = "numbers", values_to = "values")
